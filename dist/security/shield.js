...
class RuntimeShield {
    ...
    policy;
    constructor() {
        this.auditor = new auditor_1.Auditor();
        this.policy = new policy_1.PolicyEngine();
    }
    /**
     ...
     */
    async interceptRequest(toolName, args, context) {
     ...
     * @param context The authenticated user context
     */
    async interceptResult(result, context) {
    ...
    async validateSession(context) {
        if (!context.token)
            return; // public access? probably not allowed.
     ...
     */
    validateContentSafety(toolName, args) {
     ...
     */
    canonicalizePath(inputPath, context) {
        const safeBase = path_1.default.resolve(context.homeDir);
        const resolvedPath = path_1.default.resolve(safeBase, inputPath);
        if (!resolvedPath.startsWith(safeBase)) {
            throw new Error(`[Shield] Path Traversal Detected: '${inputPath}' resolves outside user scope.`);
        }
        return resolvedPath;
    }
    scanForSecrets(result) {
...
exports.RuntimeShield = RuntimeShield;
